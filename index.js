  ...
  searchedItems = searchedText => {
    ...
    if (!setSort || typeof setSort !== 'function') {
      setSort = (item, searchedText) => {
        ...
        // Ensure that the search text matches the beginning of the name
        return (
          item.name.toLowerCase().indexOf(searchedText.toLowerCase()) === 0
        );
      };
    }
  
    const filteredItems = this.props.items.filter(item => setSort(item, searchedText));
    ...
    this.setState({ listItems: filteredItems, item: item });
